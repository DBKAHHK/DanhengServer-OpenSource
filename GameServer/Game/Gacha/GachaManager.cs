...
namespace EggLink.DanhengServer.Game.Gacha
{
    public class GachaManager(PlayerInstance player) : BasePlayerManager(player)
    {
        ...
        public DoGachaScRsp? DoGacha(int bannerId, int times)
        {
            ...
            {
                var item = banner.DoGacha(GetGoldAvatars(), GetPurpleAvatars(), GetPurpleWeapons(), GetGoldWeapons(), GetBlueWeapons(), GachaData);
                items.Add(item);
            }
            ...
            foreach (var item in items)
            {
                ...
                GachaData.GachaHistory.Add(new Database.Gacha.GachaInfo()
                {
                    GachaId = bannerId,
                    ItemId = item,
