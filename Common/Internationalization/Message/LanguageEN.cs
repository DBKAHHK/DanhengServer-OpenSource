...
public class HeroTextEN
{
    ...
    public string Usage { get; } =
        "/hero <gender [1/2 - 1 for male, 2 for female]>/<type [8001/8003/8005 - corresponding to Destruction, Preservation, Harmony]>";
...
public class AvatarTextEN
{
    ...
    public string Usage { get; } =
        "/avatar <talent [character ID/-1] [path level] - set path level; character ID -1 means all owned characters>/<get [character ID] - get character>/<rank [character ID/-1] [star soul]>/<level [character ID/-1] [character level]>";
...
public class GiveAllTextEN
{
    ...
    public string Usage { get; } =
        "/giveall <avatar - character/equipment - light cone/relic - relic/unlock - luxury item> r<stack> l<level> x<quantity>";
...
public class MissionTextEN
{
    ...
    public string Usage { get; } =
        "/mission <unlockall - complete all tasks>/<pass - complete all ongoing tasks>/<finish [subtask ID] - complete specified task>/<running - get ongoing and possibly stuck tasks>/<reaccept [main task ID] - restart specified main task>";
...
public class RelicTextEN
{
    ...
    public string Usage { get; } =
        "/relic <relic ID> <main affix ID> <sub affix ID1:sub affix level> <sub affix ID2:sub affix level> <sub affix ID3:sub affix level> <sub affix ID4:sub affix level> l<level> x<quantity>";
...
public class RogueTextEN
{
    ...
    public string Usage { get; } =
        "/rogue <money [number of cosmic fragments]>/<buff [blessing ID/-1 (-1 for all blessings)]>/<miracle [curio ID]>/<enhance [blessing ID/-1]>/<unstuck - leave event>";
...
public class SceneTextEN
{
    ...
    public string Usage { get; } =
        "/scene <prop [group ID] [prop ID] [state] - set prop state>/<remove [entity ID] - remove entity>/<unlockall - unlock all props>/<change [scene entrance ID] - enter specified scene>/<reload - re-enter scene>";
...
public class MailTextEN
{
    ...
    public string Usage { get; } =
        "/mail <send [Sender Name] [Title] [Content] [Template ID] [Expire Days] - Send Mail>/<send [Sender] [Title] [Content] [Template ID] [Expire Days] [Attachments] - Send Mail with attachment>";
